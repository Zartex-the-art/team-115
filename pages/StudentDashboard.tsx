            ...
            <div className="space-y-4">
                {Object.entries(learningPath.topics).map(([category, topics]) => (
                    <div key={category} className="bg-gray-700/50 rounded-lg p-4">
                        <button onClick={() => toggleCategory(category)} className="w-full flex justify-between items-center text-left">
                                ...
                                {topics.map(topic => (
                                    <li key={topic.name} className="flex items-center">
                                        <input
                                            ...
                                            onChange={(e) => updateTopicCompletion(student.id, category, topic.name, e.target.checked)}
                                            className="w-5 h-5 text-purple-500 bg-gray-600 border-gray-500 rounded focus:ring-purple-600 cursor-pointer"
                                        />
                                        <label htmlFor={`${category}-${topic.name}`} className={`ml-3 text-lg ${topic.completed ? 'text-gray-400 line-through' : 'text-white'} cursor-pointer`}>
                                            {topic.name}
                                        </label>
                                        {topic.completed && <CheckCircleIcon className="ml-2 w-5 h-5 text-green-400"/>}
                                    </li>
                                ))}
                            </ul>
                        )}
...
const StudentDashboard: React.FC = () => {
    ...
    const { completedCount, remainingCount } = useMemo(() => {
        ...
        if (!path) return { completedCount: 0, remainingCount: 0 };
        
        const allTopics = Object.values(path.topics).flat();
        const completed = allTopics.filter(t => t.completed).length;
        return { completedCount: completed, remainingCount: allTopics.length - completed };
                       ...
                       <LearningPathComponent />
                        <div className="space-y-8">
                             <Card>
                                ...
                                <ProgressPieChart completed={completedCount} remaining={remainingCount} />
                            </Card>
                            <Card>
                                <h2 className="text-xl font-semibold mb-2">Mock Test</h2>
                                <p className="text-gray-400 mb-4">Test your knowledge with mock quizzes based on your learning path.</p>
                                <button className="w-full py-2 px-4 bg-blue-600/80 hover:bg-blue-600 rounded-lg font-semibold">Start Test</button>
                            </Card>
                            <Card>
                                <h2 className="text-xl font-semibold mb-2">AI Resume Feedback</h2>
                                <p className="text-gray-400 mb-4">Upload your resume to get feedback and suggestions.</p>
                                <button className="w-full py-2 px-4 bg-gray-600/80 hover:bg-gray-600 rounded-lg font-semibold">Upload Resume</button>
                            </Card>
                        </div>
                    </div>
                </div>
